...
router.get("/candidate", async (req, res) => {
  try {
    const getVoters = await publicityModel.find();
    res.status(200).json({
      message: "Publicity candidate found successfully",
      data: getVoters
    });
  } catch (err) {
...
router.get("/candidate/:id", async (req, res) => {
  try {
    const getVoters = await publicityModel.findById(req.params.id);
    res.status(200).json({
      message: "Publicity candidate found successfully",
      data: getVoters
    });
...
router.post("/candidate/create", verified, upload, async (req, res) => {
  try {
    if (req.user.isAdmin) {
      ...
      const createVoters = await publicityModel.create({
        name,
        voteCount,
        avatar: image.secure_url
      });
      res.status(200).json({
        message: "Publicity candidate created successfully",
        data: createVoters
      });
    } else {
      res
...
router.post("/candidate/:id", async (req, res) => {
  try {
    ...
    const createVoters = await publicityModel.findByIdAndUpdate(
      req.params.id,
      {
        $push: { voteCount }
      },
